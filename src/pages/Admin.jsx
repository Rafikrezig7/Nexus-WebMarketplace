...
export default function Admin() {
  ...
  const tabs = ['Users', 'Transactions', 'Admin Requests', 'Reports'];
  ...
  useEffect(() => {
    async function fetchReports() {
      try {
        ...
        // njbdo data ta3 les users li 3andhom relation m3a les reports
        ...
        // nmerge les data m3a les reports
        const enriched = rawReports.map(report => ({
          ...report,
          product_title:
            productsMap[report.product_id]?.title || 'Produit inconnu',
          reporter_matricule:
            profilesMap[report.reporter_id]?.matricule || 'Inconnu',
        }));
        setReports(enriched);
      } catch (err) {
        console.error(err);
  ...
  return (
    <div className="relative p-10 bg-[#F0F0F0] min-h-screen">
      ...
      {/* Tabs */}
      <div className="flex gap-4 mb-6 overflow-x-auto pb-1 hide-scrollbar">
        {tabs.map(tab => (
          <button
            key={tab}
      ...
      {activeTab === 'users' && (
        <div className="flex flex-col gap-3">
          {users.map(user => (
            <div
              key={user.id}
              className="relative bg-white rounded-2xl px-6 py-4 shadow-sm flex items-center justify-between"
            >
              {/* Left - user info */}
              <div className="flex flex-col gap-1">
                <p className="text-sm font-bold text-gray-700 font-semibold">
                  <span className="font-bold">Student/Admin Id : </span>
                  {user.matricule}
                  {user.banned && (
                    <span className="ml-2 text-xs text-red-400 font-semibold bg-red-50 px-2 py-0.5 rounded-full">
                      BANNED
                    </span>
                  )}
                </p>
            ...
            >
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-gray-400">
                  Buyer student id : {transaction.profiles?.matricule}
                </p>
                <p className="text-xs text-gray-400">
                  Buyer ID: {transaction.buyer_id}
                </p>
                <p className="text-xs text-gray-400">
                  Seller Id : {transaction.products?.seller_id}
                </p>
                <p className="text-xs text-gray-400">
        ...
        <div className="flex flex-col gap-3">
          {reports.length === 0 ? (
            ...
            reports.map(report => (
              <div
                key={report.id}
                className="bg-white rounded-2xl px-6 py-4 shadow-sm flex flex-col md:flex-row md:justify-between md:items-center gap-4"
              >
                <div className="flex flex-col gap-1 flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    Reporter: {report.reporter_matricule} (ID:{' '}
                    {report.reporter_id?.slice(0, 8)}...)
                  </p>
                  <p className="text-xs text-gray-500">
                    Date: {new Date(report.created_at).toLocaleDateString()}
                  </p>
                  <p className="text-xs text-gray-700 mt-1 bg-gray-50 p-2 rounded break-words">
                    {report.reason}
                  </p>
                </div>
                <div className="flex items-center gap-2 self-end md:self-auto">
                  <button
                    ...
                    className="flex items-center gap-1 text-xs font-semibold text-white bg-red-400 hover:bg-red-500 px-3 py-1.5 rounded-full transition-colors duration-200 whitespace-nowrap"
                  >
                    <IoRemoveCircleOutline size={14} /> Delete
                  </button>
                </div>
      ...
      <div
        ...
        className="absolute top-3 left-3 flex items-center text-pink-800 cursor-pointer"
      >
