      ...
      >
        {({ isActive }) => (
          <>
            ...
            <span
              className={`hidden sm:inline ${isActive ? 'text-white font-semibold' : ''}`}
            >
              {children}
            </span>
            {isActive && (
...
export default function Dashboard() {
  ...
  // 2. Fetch / create profile after authUser is available
  useEffect(() => {
    if (!authUser) return;
    ...
    async function fetchProfile() {
      ...
      if (error || !data) {
        ...
        const { data: newProfile, error: insertError } = await supabase
          .from('profiles')
          .insert([{ id: authUser.id, username: authUser.email.split('@')[0] }])
          .select()
          .single();
        if (insertError) {
  ...
  // Admin request – shows message if already admin
  async function handleAdminRequest() {
    ...
    if (error) {
      ...
      alert('Application sent. Check contacts for more info!');
    }
  }
  ...
  // Avatar: first two letters of username (fallback to email, then '??')
      ...
      <div className="sticky top-0 z-50">
        ...
        <nav className="flex justify-between items-center py-3 px-6 md:px-20 bg-white shadow-sm border-b border-gray-200 gap-4">
          <div
            ...
            className="cursor-pointer font-bold flex-none text-3xl tracking-tight"
          >
            NEX
              ...
              <AnimatePresence>
                {showDropdown && (
                  ...
                  >
                    ...
                    {/* Apply for Admin – modified to handle admin state */}
                    <button
                      onClick={handleAdminRequest}
                      disabled={isAdmin}
                      className={`text-sm text-left py-1 transition-colors duration-200 ${
                        isAdmin
                          ? 'text-gray-400 cursor-default'
                          : 'text-gray-600 hover:text-[#FF4760]'
                      }`}
                    >
                      {isAdmin ? 'You are admin ✓' : 'Apply for Admin'}
                    </button>
